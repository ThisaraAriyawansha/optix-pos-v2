                ...
                <div onclick="window.location='{{ route('branches.edit', $branch) }}'"
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                            <a href="{{ route('branches.edit', $branch) }}" onclick="event.stopPropagation()"
                               class="btn-icon-brand w-8 h-8 flex items-center justify-center rounded-lg bg-surface-alt border border-subtle text-gray-700 dark:text-gray-200 active:scale-90 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                            </a>
                            <form action="{{ route('branches.toggleStatus', $branch) }}" method="POST" onclick="event.stopPropagation()">
                                @csrf
                                @method('PATCH')
