...
<main class="px-5 pt-5 pb-28 max-w-2xl mx-auto">
    ...
    <form action="{{ route('customers.update', $customer) }}" method="POST" class="rounded-2xl bg-surface border border-subtle p-5 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" value="{{ $customer->customer_code }}" disabled
                   class="w-full px-4 py-2.5 rounded-xl bg-surface-alt border border-subtle text-sm font-sans text-gray-400 dark:text-gray-500 cursor-not-allowed">
        </div>
